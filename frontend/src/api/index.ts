...
/**
 * Fetch GIS layers (footpath, cycling path, shared path) around a specific point
 * @param project - Project name
 * @param point - [lon, lat] coordinates
 * @param radius - Search radius in meters
 * @param layers - Array of layer names to fetch
 * @returns GIS layer features
 */
...
export async function fetchGISLayers(
  ...
  const res = await fetch(
    `/api/projects/${encodeURIComponent(project)}/gis/layers`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
